    ...
    for id_ in tqdm_ids:
        ...
        for suffix, m in [('tr', 'Training'), ('vl', 'Validation'), ('ts', 'Test')]:
            ...
            for k,v in tmp.items():
                ...
                aus[f'{suffix} {k} mean'] = np.mean(v) if not 'confusion_matrix' in k else 0#np.mean(v, axis=0)
                aus[f'{suffix} {k} std'] = np.std(v) if not 'confusion_matrix' in k else 0#np.std(v, axis=0)
            result.update(aus)
