...
from custom_graphgym.loss.subtoken_prediction_loss import subtoken_cross_entropy
from custom_graphgym.utils import cfg_to_dict, flatten_dict, make_wandb_name


def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation,device):
    model.train()
    optimizer.zero_grad()
    time_start = time.time()
    for iter, batch in enumerate(loader):
        batch.split = 'train'
        #batch.edge_index = SparseTensor(row=batch.edge_index[0], col=batch.edge_index[1])
        batch.to(device)
        ...
        loss_unreg = loss.detach().cpu().item()
...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        train_epoch(loggers[0], loaders[0], model, optimizer, scheduler,
                    cfg.optim.batch_accumulation,device)
